		...
		socket.on('newMessage', function(roomId, message) {
			...
			Room.findById(roomId, function(err, room){
				...
				if(!room){
					...
					Room.addMessage(room, message, socket, function(err, Message){

                        socket.emit('addMessage', message, Message.messages[Message.messages.length - 1]);
                        socket.broadcast.to(roomId).emit('addMessage', message, Message.messages[Message.messages.length - 1]);
